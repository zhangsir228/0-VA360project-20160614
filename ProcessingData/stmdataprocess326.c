...
float voltage_effective_tab[64]={0},current_effective_tab[64]={0},active_power_tab[64]={0}; //2016-07-25 lea  换用窗口滤波 加快数值更新速度 
float voltage_mean_tab[64]={0},current_mean_tab[64]={0};
char  Window_num=40;//窗口滤波大小;
		...
		{//处理数据数组		
			if(RotaryKeyValue==KEY_VALUE_6)//V+A 档位  
			{
				...
				{					
					if(rangenum ==0)
					{
						if(Sysflag.Calonce>0)//校准模式
						{
							if(Sysflag.Calonce==1)//零点
							{
							...
							else//增益
							{
								...
								if(SaveData.Value.cal_6VD_gain<0)SaveData.Value.cal_6VD_gain=1;
...
//		active_power_sum += power_temp;		
		...
		count=5;//原来为多次测量后计算一次取平均值，换到统一函数做窗口滤波。
		//count++;
		...
		*********************************************/
		...
		{//VA档的THD%r、THD%f档，W档的Param档
			if(fft_count>=1)						//每做fft_count次基本运算，就做一次FFT
			{
				...
				voltage_foudamental_effective = voltage_effective;
				...
				current_foudamental_effective = current_effective;
				...
				power_foudamental_effective = active_power;
		...
		else if(RotaryKeyValue == KEY_VALUE_7)//W 档位
		{
			VadENA=0;//SDADC非使能标志传递到比较中中断，过零点开始ADC采集 由TIM5 比较器中断里面触发定时器
...
//快速排序法
void quick(float *a,int i,int j) 
{ 
	int m,n; 
	float k,temp; 
...
float Windows_Filter(float New_Data,float *Data_tab,char num)
{
	...
	if((Data_tab==current_mean_tab)||(Data_tab==current_effective_tab))//电流滤波的容许误差放大一点
	{
		if(fabs(New_Data-Data_tab[num/2]) > 2.5/*fabs(Data_tab[num-1]*0.08)*/)//若偏差值较大则填充滤波寄存器
		{
			for(i=0;i<num;i++)
			{
				Data_tab[i]=New_Data;
	...
	{
		if(fabs(New_Data-Data_tab[num/2]) > (0.02*pow(10,rangenum))/*fabs(Data_tab[num-1]*0.02)*/)//若偏差值超过当前档位的二十个字。
		{
			for(i=0;i<num;i++)
			{
				Data_tab[i]=New_Data;
