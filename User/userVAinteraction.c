...
void Data_init(void)//包含MCU测量电流电压值传递给相对值
{
	...
	if(Is_Cal_Mode == 0)//20160909 修改 校准模式下不清空电压电流值
	{
		voltage_effective=0;
		voltage_mean=0;
		current_effective=0;
...
//							ShortKey_flag=0;//后面改变功能要用
					...
					{
						if(Is_Cal_Mode == 1)//校准模式下REL作为DCV零点校准功能
						{
					...
					{
						if((ShortKey_flag == 1) && (peakstatus == state0) && (phasestatus == state0) && (holdstatus==state0) && (funcstatus == state0))
						{//在PARAM出现短按，并且处于非Peak、Phase、HOLD状态下  20160909 新增 AC档下
							ShortKey_flag=0;
							max_minstatus=state0;
							relstatus=state0;
							rangestatus = state0;
...
//							ShortKey_flag=0;//后面改变功能要用
					...
					case KEY_VALUE_2://REL
					{
						if(Is_Cal_Mode == 1)//校准模式下REL作为VmV零点校准功能
						{
