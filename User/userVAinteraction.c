...
/*测量功能选择——当短按键Func时，DTA0660测量功能改变*/
					...
					case KEY_VALUE_2://REL
					{
						...
						{//在REL出现短按，并且处于非Peak、Phase、HOLD状态下
							...
							if(relstatus != state0)//在相对值状态读一个测量值
							{
								...
								{
									rel_data1=voltage_mean;
									rel_data2=current_mean;
								}
								else if(funcstatus ==state2)//AC+DC V+A
								{
									rel_data1=voltage_effective;
									rel_data2=current_effective;
								}
								
								//此处处理DTA测量得到的电压量程，数据归正
								if(rangenum == 0)
									rel_data1*=10;
								else if(rangenum == 1)
									rel_data1*=100;
...
//							ShortKey_flag=0;//后面改变功能要用
					...
					case KEY_VALUE_4://PARAM
					{
						if((ShortKey_flag == 1) && (funcstatus == state1) && (holdstatus==state0))
...
//						else if(rangenum == 2)
//							readstmdata1*=1000;
						//qq readstmdata1/=pow(10,2-Sysflag.DTArange); 
						showdata1=deal_1(readstmdata1,1);
						showdata2=deal_1(readstmdata2,2);
