...
extern void display_bt_state(void);/*显示bluetooth状态*/

char tempR[50],tempS[50],tempT[50],tempU[50],tempV[50],tempW[50];

uint8_t IsReading = 0;//是否在持续读取模式下
uint8_t T2s = 0;	//两秒间隔连续输出标志
char     Tstr[128] = {0};//待发送数组  在userVAinteraction中根据显示部分填充。
...
const char *CMDmap2[100]={
          ...
          "state",		"state",//20 program_out数组当前状态
					...
					"tcb",			"tcb",//46
					"tcn",			"tcn",//48
					"initialflash",			"tcx",//50 初始化校准参数  51没用
      ...
      break;
		...
		case 9://调试，读取电流1024个数据。
		{
			switch(tempT1)
			{
				...
				case 16://16 read:end 结束读取命令
				{
					IsReading = 0;
					break;
				}
				
				case 22://22 read:start 持续读取命令
				{
					IsReading = 1;
					break;
				}
...
//			"cal_adjv",				//80
		...
		case 30://30 控制蓝牙开关	"bt_pwr",
		{
			...
			if(input_data == 1)
			{
				btstatus = state1;
				Power_On_Bt();
				strcat(str,"Power_On_Bt!");
			}
			else
			{
				btstatus = state0;
				Power_Off_Bt();
				strcat(str,"Power_Off_Bt!");
			}
			display_bt_state();//显示蓝牙符号
			break;
		}
		default:break;
...
void Communication_Service()
{
	...
	if(T2s>=2)//此处为数据输出间隔 0.5*4=2S
	{
		T2s=0;
		if(IsReading==1)//若允许持续输出
		{
			if(Tstr[0] != 0x00)
			{
				printf(Tstr);	
...
//串口输出 
//将待输出字符串整合到数组中
void BT_format(float data,char* danwei,char* str)
{
	sprintf(str,"%.4",data);
	strcat(str," ");
	strcat(str,danwei);
