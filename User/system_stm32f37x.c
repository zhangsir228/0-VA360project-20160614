...
static void SetSysClock(void)
{
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
	...
	//如果HSE没有启动到72M（外部晶振12M），则使用HSI倍频到72M
  else
  { /* If HSE fails to start-up, the application will have wrong clock 
    configuration. User can add here some code to deal with this error */
		...
		FLASH_PrefetchBufferCmd(ENABLE);        //开启FLASH预取指功能
		//FLASH时序控制
		//推荐值:SYSCLK = 0~24MHz   Latency=0
		//       SYSCLK = 24~48MHz  Latency=1
		//       SYSCLK = 48~72MHz  Latency=2
		FLASH_SetLatency(FLASH_Latency_2);
		RCC_HCLKConfig(RCC_SYSCLK_Div1);        //设置HCLK(AHB时钟)=SYSCLK=72
		RCC_PCLK2Config(RCC_HCLK_Div1);                //PCLK2(APB2) = HCLK=72
		RCC_PCLK1Config(RCC_HCLK_Div2);                //PCLK1(APB1) = HCLK/2=36

		//PLL设置 SYSCLK/2 * 12 = 4*16 = 64MHz
